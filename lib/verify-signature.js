 ...
 */
function verifySignature(body, headers, isDns, callback) {
  var signature = null;
  ...
  DKIM.getKey(signature.domain, signature.selector, isDns, (error, key) => {
    if (error != null) {
      result.error = error;
    ...
    var digest = crypto
      .createHash(signature.algorithm.split("-").pop())
      .update(message, "latin1")
      .digest();

    // do not check body hash
    ...
    try {
      verified = crypto
        .createVerify(signature.algorithm.toUpperCase())
